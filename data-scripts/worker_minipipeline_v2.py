...
if __name__ == "__main__":
    ...
    if args["mode"] == "config":
        ...
        for index, czi in df.iterrows():
            ...
            print("Loading ", czi["czi_name"], "...")

            #
            # Checking if the CZI file exist
            #

            czi_path = os.path.join(czi["czi_path"], czi["czi_name"])

            if not os.path.isfile(czi_path):
                raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), os.path.join(czi["czi_path"], czi["czi_name"]))
            ...
            list_of_suffix = ast.literal_eval(czi["seg_suffix"])
            list_of_channel = ast.literal_eval(czi["channel"])

            for suffix_id, suffix in enumerate(list_of_suffix):
                print(suffix_id, suffix, list_of_channel[suffix_id])

            filenames = glob.glob1(seg_path, czi["czi_name"].replace(".czi","") + "*nucWholeIndexImageScale.tiff")
            filenames.sort()
